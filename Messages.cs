...
public static class Messages
{
    ...
    private static DiscordEmbedBuilder.EmbedAuthor Author =
        new()
        {
            Name = "Vroom",
            IconUrl =
                "https://cdn.discordapp.com/avatars/1217477160340815985/f1f53c4ec2396c9dce125ab1611620d8.webp",
    ...
    public static DiscordWebhookBuilder Help(SlashCommandsExtension slash)
    {
        ...
        {
            embed.AddField($"</{cmd.Name}:{cmd.Id}>", "> " + GetDescription(cmd.Name));
        }
    ...
    private static DiscordWebhookBuilder AddButtonsToWebhook(
    ...
    )
    {
        webhook.AddComponents(
            new DiscordComponent[]
            {
                new DiscordButtonComponent(
                    IsSelected(StatType.Overall, current),
                    StatType.Overall.ToString() + $"_{userId}",
                    "Overall"
                ),
                new DiscordButtonComponent(
                    IsSelected(StatType.Vs1, current),
                    StatType.Vs1.ToString() + $"_{userId}",
                    "1v1"
                ),
                new DiscordButtonComponent(
                    IsSelected(StatType.Vs2, current),
                    StatType.Vs2.ToString() + $"_{userId}",
                    "2v2"
                ),
                new DiscordButtonComponent(
                    IsSelected(StatType.Vs3, current),
                    StatType.Vs3.ToString() + $"_{userId}",
                    "3v3"
                ),
                new DiscordButtonComponent(
                    IsSelected(StatType.Rumble, current),
                    StatType.Rumble.ToString() + $"_{userId}",
                    "Rumble"
                ),
            }
        );
        webhook.AddComponents(
            ...
            {
                new DiscordButtonComponent(
                    IsSelected(StatType.Casual, current),
                    StatType.Casual.ToString() + $"_{userId}",
                    "Casual"
                ),
                new DiscordButtonComponent(
                    IsSelected(StatType.Hoops, current),
                    StatType.Hoops.ToString() + $"_{userId}",
                    "Hoops"
                ),
                new DiscordButtonComponent(
                    IsSelected(StatType.Dropshot, current),
                    StatType.Dropshot.ToString() + $"_{userId}",
                    "Dropshot"
                ),
                new DiscordButtonComponent(
                    IsSelected(StatType.Snowday, current),
                    StatType.Snowday.ToString() + $"_{userId}",
                    "Snowday"
                ),
            }
        );
    ...
    private static DiscordInteractionResponseBuilder AddButtonsToInteraction(
        DiscordInteractionResponseBuilder interaction,
        ulong userId,
        StatType current
    )
    {
        interaction.AddComponents(
            new DiscordComponent[]
            {
                new DiscordButtonComponent(
                    IsSelected(StatType.Overall, current),
    ...
    public static async Task<DiscordWebhookBuilder> Stats(
        Database db,
        ulong userId,
        Query query,
        StatType selection,
    ...
    public static async Task<DiscordInteractionResponseBuilder> InteractionStats(
        Database db,
        ulong userId,
        Query query,
        StatType selection
    ...
    private static async Task<DiscordEmbedBuilder> OverallStats(
        Database db,
        ulong userId,
        Query query,
        StatType selection
    )
    {
        ...
        embed.AddField(":crown: Wins", $"{stats.Wins}", true);

        embed.AddField(":soccer: Shots", $"{stats.Shots}", true);

        embed.AddField(":shield: Saves", $"{stats.Saves}", true);

        embed.AddField(":handshake: Assists", $"{stats.Assists}", true);

        embed.AddField(":goal: Goals", $"{stats.Goals}", true);

        embed.AddField(":star: MVPs", $"{stats.MVPs}", true);

        embed.AddField(":gift: Reward Level", stats.RewardLevel.ToString().Replace("2", ""), true);

        embed.AddField(":eyes: Profile Views", $"{stats.ProfileViews}", true);
    ...
    private static async Task<DiscordEmbedBuilder> ModeStats(
        Database db,
        ulong userId,
        Query query,
        StatType playlist
    )
    {
        ...
        embed.AddField(":trophy: Rank", rank, true);

        embed.AddField(":ladder: Division", $"{mode.Division}", true);

        embed.AddField(":bar_chart: MMR", $"{mode.MMR}", true);
    ...
    public static DiscordWebhookBuilder StatAccountNotFound(
    ...
    )
    {
        ...
        embed.Description =
            $"Stats for **{attemptedUsername}** on **{platform.ToString()}** not found."
